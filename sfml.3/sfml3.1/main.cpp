...
void update(const sf::Vector2f &mousePosition, sf::ConvexShape &pointer, float time)
{
    ...
    float angle = toDegress(atan2(delta.y, delta.x));
    float DeltaAngle = maxAngularSpeed * time;
    float toMove = 1;
    if (angle < 0)
    {
        angle = 360 + angle;
    ...
    {
        if (pointer.getRotation() - angle < 180)
        {
            toMove = -1;
        }
    }
    else if (angle > pointer.getRotation())
    {
    ...
    pointer.setRotation(DeltaAngle * toMove + pointer.getRotation());
}
