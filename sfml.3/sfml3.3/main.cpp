...
constexpr float RadiusPupil = 20;
...
const sf::Color ColorPupil = sf::Color(0x0, 0x0, 0x0);
...
{
	float x1 = (WidthEye - DistBetEyeAndPupil) * std::cos(angle);
	float x2 = (WidthEye - DistBetEyeAndPupil) * std::cos(angle + M_PI);
	float y1 = (HeightEye - DistBetEyeAndPupil) * std::sin(angle);
	float y2 = (HeightEye - DistBetEyeAndPupil) * std::sin(angle + M_PI);
...
void Eye::update(sf::Vector2f &mousePosition)
{
	...
	if (0 == this->mouseOnEye(delta, angle))
	{
		...
		this->eyePupil.setPosition(this->position + offset);
	}
	else
	{
...
void Eye::init(sf::Vector2f position, sf::Vector2f size)
{
	...
	this->eyeApple.setFillColor(sf::Color(0xFF, 0xFF, 0xFF));
	this->eyePupil.setFillColor(sf::Color(0x0, 0x0, 0x0));
}
